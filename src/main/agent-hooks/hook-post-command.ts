...
export function buildPosixAgentHookPostCommand(
  ...
  return [
    ...
    `  orca_hook_metadata=$(printf '%s\\037%s\\037%s\\037%s\\037%s\\037%s\\037%s\\037%s' "$ORCA_PANE_KEY" "$ORCA_TAB_ID" "$ORCA_AGENT_LAUNCH_TOKEN" "$ORCA_WORKTREE_ID" "$ORCA_AGENT_HOOK_ENV" "$ORCA_AGENT_HOOK_VERSION" "$PWD" | base64 | tr -d '\\n') && \\`,
    ...
    `  ${indent}--data-urlencode "hookCwd=\${PWD}" \\`,
